...
def mock_redis_client():
    ...
    client.publish = Mock(return_value=1)  # Return value indicating success
    ...
    # Add nested 'client' attribute for peer discovery
    mock_inner_client = Mock()
    mock_inner_client.keys = Mock(return_value=[])
...
class TestFRLVDNInitialization:
    ...
    def test_agent_with_both_engines(self, mock_mesa_model, mock_redis_client):
        ...
        assert agent.unique_id is not None  # ID is auto-generated by Mesa
...
class TestFRLPolicySharing:
    ...
    def test_policy_registration(self, mock_redis_client):
        ...
        # Connect to a peer first (required for sharing)
        engine.connect_to_peer("agent_2")
        ...
        # Register policy
        result = engine.share_policy(policy, performance_score=0.75)

        # Verify policy was shared (via Redis publish)
        assert result > 0  # Should have shared with at least one peer
    ...
    def test_peer_discovery(self, mock_redis_client):
        ...
        # Simulate peer discovery - discover_peers uses client.keys()
        mock_redis_client.client.keys.return_value = [b"agent:state:agent_2", b"agent:state:agent_3"]

        discovered_peers = engine1.discover_peers()

        # Should discover peers (excluding self)
        assert len(discovered_peers) >= 0
        assert mock_redis_client.publish.called  # Announces presence
        assert mock_redis_client.client.keys.called  # Queries for peers
    ...
    def test_multi_agent_collaboration_workflow(self, mock_mesa_model, mock_redis_client):
        ...
        for i in range(3):
            agent = SpecialistAgent(
                model=mock_mesa_model,
                redis_client=mock_redis_client,
                data_channel=f"test_channel_{i}",
